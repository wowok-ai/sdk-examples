...
function compose(e) {
  ...
  var r = pipe(fromAsyncIterable(async function* convertError(e) {
    try {
      ...
      yield !(e instanceof CLIError) ? ("" + e).trim() + "\n" : e;
    }
    yield "\n";
...
function diagnosticMessage(e) {
  ...
  return text([ ke, cmd(oe.Style, de.BrightBlack), `${e.line}:${e.col}`, pe.Tab, cmd(oe.Style, t), e.severity, pe.Tab, cmd(oe.Style, de.Foreground), indent(e.message.trim(), text([ ke, pe.Tab, pe.Tab ])), pe.Newline ]);
}
...
var stat = async (e, t = Ne.File) => await S.stat(e).then((e => {
  switch (t) {
   case Ne.File:
    return e.isFile();
...
var Pe = /^(?:graphql\.config|\.graphqlrc)\.(?:cjs|[jt]s|json|toml|ya?ml)$/i;
...
var findGraphQLConfig = async e => {
  ...
  while (t !== r) {
    ...
    var i = n.find((e => Pe.test(e)));
    if (i) {
      return i;
    }
    if (await stat(B.resolve(t, ".git"), Ne.Directory)) {
      break;
    } else if (await stat(B.resolve(t, ".vscode"), Ne.Directory)) {
      break;
    }
    t = B.resolve(t, "..");
  }
...
var isVSCodeInstalled = async () => {
  ...
  return !!await stat(e, Ne.Directory);
};
...
async function* run$1() {
  ...
  yield runningTask(Me.CHECK_TS_VERSION);
  await delay();
  var n = process.cwd();
  ...
  try {
    ...
    yield failedTask(Me.CHECK_TS_VERSION);
  ...
  yield completedTask(Me.CHECK_TS_VERSION);
  yield runningTask(Me.CHECK_DEPENDENCIES);
  await delay();
  var l = D.find((e => "@0no-co/graphqlsp" === e[0]));
  if (!l) {
    yield failedTask(Me.CHECK_DEPENDENCIES);
  ...
  yield* async function* runVSCodeChecks() {
    ...
    if (t.length || r) {
      ...
      if (!n) {
        yield completedTask(Me.CHECK_VSCODE);
      }
    }
  }();
  yield runningTask(Me.CHECK_SCHEMA);
  await delay();
  try {
    await t.loadRef(h).load({
      rootPath: e.dirname(f.configPath)
    });
  } catch (e) {
    yield failedTask(Me.CHECK_SCHEMA);
    throw externalError("Failed to load schema.", e);
  }
  yield completedTask(Me.CHECK_SCHEMA, !0);
  await delay();
...
var _e = {
  ...
  forward: (e = 1) => `${Re}${e}C`,
  backward: (e = 1) => `${Re}${e}D`,
  nextLine: (e = 1) => `${Re}E`.repeat(e),
  prevLine: (e = 1) => `${Re}F`.repeat(e),
  left: `${Re}G`,
  hide: `${Re}?25l`,
  show: `${Re}?25h`,
  save: "7",
...
var Ue = {
  screen: `${Re}2J`,
  up: (e = 1) => `${Re}1J`.repeat(e),
  down: (e = 1) => `${Re}J`.repeat(e),
  line: `${Re}2K`,
  lineEnd: `${Re}K`,
  lineStart: `${Re}1K`,
  lines(e) {
...
var Ge = {
  cursor: _e,
  scroll: {
    up: (e = 1) => `${Re}S`.repeat(e),
    down: (e = 1) => `${Re}T`.repeat(e)
  },
  erase: Ue,
  beep: ""
};
...
qe.exports.createColors = create;

var Ve = getDefaultExportFromCjs(qe.exports);
...
!function(e) {
  var t = {};
  He.exports = t, t.eastAsianWidth = function(e) {
...
var T = (e = 0) => t => `[${t + e}m`, P = (e = 0) => t => `[${38 + e};5;${t}m`, W = (e = 0) => (t, r, n) => `[${38 + e};2;${t};${r};${n}m`, ze = {
  modifier: {
    reset: [ 0, 0 ],
    bold: [ 1, 22 ],
    dim: [ 2, 22 ],
...
Object.keys(ze.modifier);

Object.keys(ze.color), Object.keys(ze.bgColor);

var Je = function tD() {
  var e = new Map;
  for (var [t, r] of Object.entries(ze)) {
    for (var [n, i] of Object.entries(r)) {
      ze[n] = {
        open: `[${i[0]}m`,
        close: `[${i[1]}m`
      }, r[n] = ze[n], e.set(i[0], i[1]);
    }
  ...
  return Object.defineProperty(ze, "codes", {
    value: e,
    enumerable: !1
  }), ze.color.close = "[39m", ze.bgColor.close = "[49m", ze.color.ansi = T(), ze.color.ansi256 = P(), 
  ze.color.ansi16m = W(), ze.bgColor.ansi = T(10), ze.bgColor.ansi256 = P(10), ze.bgColor.ansi16m = W(10), 
  Object.defineProperties(ze, {
    ...
    hexToAnsi256: {
      value: e => ze.rgbToAnsi256(...ze.hexToRgb(e)),
      enumerable: !1
    },
    ansi256ToAnsi: {
      value: e => {
    ...
    rgbToAnsi: {
      value: (e, t, r) => ze.ansi256ToAnsi(ze.rgbToAnsi256(e, t, r)),
      enumerable: !1
    },
    hexToAnsi: {
      value: e => ze.ansi256ToAnsi(ze.hexToAnsi256(e)),
      enumerable: !1
    }
  }), ze;
}(), Xe = new Set([ "", "" ]), Ze = "]8;;", N = e => `${Xe.values().next().value}[${e}m`, L = e => `${Xe.values().next().value}${Ze}${e}`, y$1 = (e, t, r) => {
  var n = [ ...t ];
  var i = !1, a = !1, s = A(S$1(e[e.length - 1]));
  for (var [D, c] of n.entries()) {
    var l = A(c);
    if (s + l <= r ? e[e.length - 1] += c : (e.push(c), s = 0), Xe.has(c) && (i = !0, 
  ...
  for (var [p, m] of h.entries()) {
    if (a += m, Xe.has(m)) {
      var {groups: g} = new RegExp(`(?:\\[(?<code>\\d+)m|\\${Ze}(?<uri>.*))`).exec(h.slice(p).join("")) || {
        groups: {}
      };
      if (void 0 !== g.code) {
...
var Qe = Object.defineProperty, a$1 = (e, t, r) => (((e, t, r) => {
  t in e ? Qe(e, t, {
    enumerable: !0,
    configurable: !0,
    writable: !0,
...
var tt = new Map([ [ "k", "up" ], [ "j", "down" ], [ "h", "left" ], [ "l", "right" ] ]), rt = new Set([ "up", "down", "left", "right", "space", "enter" ]);
...
let nt = class x {
  ...
  prompt() {
    ...
    this.render(), new Promise(((e, t) => {
      this.once("submit", (() => {
        this.output.write(Ge.cursor.show), this.output.off("resize", this.render), v(this.input, !1), 
        e(this.value);
      })), this.once("cancel", (() => {
        this.output.write(Ge.cursor.show), this.output.off("resize", this.render), v(this.input, !1), 
        e(et);
  ...
  onKeypress(e, t) {
    if ("error" === this.state && (this.state = "active"), t?.name && !this._track && tt.has(t.name) && this.emit("cursor", tt.get(t.name)), 
    t?.name && rt.has(t.name) && this.emit("cursor", t.name), e && ("y" === e.toLowerCase() || "n" === e.toLowerCase()) && this.emit("confirm", "y" === e.toLowerCase()), 
  ...
  restoreCursor() {
    ...
    this.output.write(Ge.cursor.move(-999, -1 * e));
  }
  render() {
    ...
    if (e !== this._prevFrame) {
      if ("initial" === this.state) {
        this.output.write(Ge.cursor.hide);
      } else {
        ...
        if (this.restoreCursor(), t && 1 === t?.length) {
          var r = t[0];
          this.output.write(Ge.cursor.move(0, r)), this.output.write(Ge.erase.lines(1));
          var n = e.split("\n");
          this.output.write(n[r]), this._prevFrame = e, this.output.write(Ge.cursor.move(0, n.length - r - 1));
          return;
        } else if (t && t?.length > 1) {
          var i = t[0];
          this.output.write(Ge.cursor.move(0, i)), this.output.write(Ge.erase.down());
          var a = e.split("\n").slice(i);
          this.output.write(a.join("\n")), this._prevFrame = e;
          return;
        }
        this.output.write(Ge.erase.down());
      }
...
class xD extends nt {
  ...
  constructor(e) {
    super(e, !1), this.value = !!e.initialValue, this.on("value", (() => {
      ...
      this.output.write(Ge.cursor.move(0, -1)), this.value = e, this.state = "submit", 
      this.close();
    })), this.on("cursor", (() => {
...
var it = Object.defineProperty, MD = (e, t, r) => (((e, t, r) => {
  t in e ? it(e, t, {
    enumerable: !0,
    configurable: !0,
    writable: !0,
  ...
  constructor(e) {
    super(e), MD(this, "valueWithCursor", ""), this.on("finalize", (() => {
      ...
      if (this.cursor >= this.value.length) {
        this.valueWithCursor = `${this.value}${Ve.inverse(Ve.hidden("_"))}`;
      } else {
        var e = this.value.slice(0, this.cursor), t = this.value.slice(this.cursor);
        this.valueWithCursor = `${e}${Ve.inverse(t[0])}${t.slice(1)}`;
      }
    }));
  }
  get cursor() {
...
var ut = function q() {
...
}(), o = (e, t) => ut ? e : t, ot = o("◆", "*"), st = o("■", "x"), Dt = o("▲", "x"), ct = o("◇", "o"), lt = o("┌", "T"), dt = o("│", "|"), ft = o("└", "—"), ht = o("●", ">"), pt = o("○", " "), y = e => {
  switch (e) {
   case "initial":
   case "active":
    return Ve.cyan(ot);
  ...
  render() {
    var t = `${Ve.gray(dt)}\n${y(this.state)}  ${e.message}\n`, r = e.placeholder ? Ve.inverse(e.placeholder[0]) + Ve.dim(e.placeholder.slice(1)) : Ve.inverse(Ve.hidden("_")), n = this.value ? this.valueWithCursor : r;
    switch (this.state) {
     case "error":
      return `${t.trim()}\n${Ve.yellow(dt)}  ${n}\n${Ve.yellow(ft)}  ${Ve.yellow(this.error)}\n`;

     case "submit":
      return `${t}${Ve.gray(dt)}  ${Ve.dim(this.value || e.placeholder)}`;

     case "cancel":
      return `${t}${Ve.gray(dt)}  ${Ve.strikethrough(Ve.dim(this.value ?? ""))}${this.value?.trim() ? "\n" + Ve.gray(dt) : ""}`;

     default:
      return `${t}${Ve.cyan(dt)}  ${n}\n${Ve.cyan(ft)}\n`;
    }
  }
}).prompt(), se = e => {
  ...
  return new xD({
    ...
    render() {
      ...
      switch (this.state) {
       case "submit":
        return `${n}${Ve.gray(dt)}  ${Ve.dim(i)}`;

       case "cancel":
        return `${n}${Ve.gray(dt)}  ${Ve.strikethrough(Ve.dim(i))}\n${Ve.gray(dt)}`;

       default:
        return `${n}${Ve.cyan(dt)}  ${this.value ? `${Ve.green(ht)} ${t}` : `${Ve.dim(pt)} ${Ve.dim(t)}`} ${Ve.dim("/")} ${this.value ? `${Ve.dim(pt)} ${Ve.dim(r)}` : `${Ve.green(ht)} ${r}`}\n${Ve.cyan(ft)}\n`;
      }
    }
  }).prompt();
}, ue = (e = "") => {
  process.stdout.write(`${Ve.gray(ft)}  ${Ve.red(e)}\n\n`);
};
...
var wt = "win32" === process.platform || "cygwin" === process.env.OSTYPE || "msys" === process.env.OSTYPE;
...
var getPathInfo = (e, t) => {
  var r = t.colon || Bt;
  var n = e.match(/\//) || wt && e.match(/\\/) ? [ "" ] : [ ...wt ? [ process.cwd() ] : [], ...(t.path || process.env.PATH || "").split(r) ];
  var i = wt ? t.pathExt || process.env.PATHEXT || ".EXE;.CMD;.BAT;.COM" : "";
  var a = wt ? i.split(r) : [ "" ];
  if (wt) {
    if (-1 !== e.indexOf(".") && "" !== a[0]) {
      a.unshift("");
...
function resolveCommandAttempt(e, t) {
  ...
  if (s) {
    s = At.resolve(i ? e.options.cwd : "", s);
  }
  return s;
}

var kt = function resolveCommand$1(e) {
...
It.command = function escapeCommand(e) {
  return e = e.replace(Nt, "^$1");
};

It.argument = function escapeArgument(e, t) {
  e = (e = `"${e = (e = (e = `${e}`).replace(/(\\*)"/g, '$1$1\\"')).replace(/(\\*)$/, "$1$1")}"`).replace(Nt, "^$1");
  if (t) {
    e = e.replace(Nt, "^$1");
  }
  return e;
...
var _t = function readShebang$1(e) {
  var t = Buffer.alloc(150);
  var r;
  try {
    r = Mt.openSync(e, "r");
    Mt.readSync(r, t, 0, 150, 0);
    Mt.closeSync(r);
  } catch (e) {}
  return shebangCommand(t.toString());
};

var Ut = "win32" === process.platform;

var Gt = /\.(?:com|exe)$/i;

var qt = /node_modules[\\/].bin[\\/][^\\/]+\.cmd$/i;
...
function parseNonShell(e) {
  ...
  if (e.options.forceShell || r) {
    ...
    e.command = Rt.command(e.command);
    e.args = e.args.map((e => Rt.argument(e, n)));
...
function spawn(e, t, r) {
  var n = Kt(e, t, r);
  var i = Ht.spawn(n.command, n.args, n.options);
  Yt.hookChildProcess(i, n);
  return i;
}

vt.exports = spawn;

vt.exports.spawn = spawn;

vt.exports.sync = function spawnSync(e, t, r) {
  var n = Kt(e, t, r);
  var i = Ht.spawnSync(n.command, n.args, n.options);
  i.error = i.error || Yt.verifyENOENTSync(i.status, n);
  return i;
};

vt.exports._parse = Kt;

vt.exports._enoent = Yt;

var zt = getDefaultExportFromCjs(vt.exports);
...
var Jt = Object.getOwnPropertyDescriptor(Function.prototype, "toString");

var Xt = Object.getOwnPropertyDescriptor(Function.prototype.toString, "name");
...
function mimicFunction(e, t, {ignoreNonConfigurable: r = !1} = {}) {
  ...
  ((e, t, r) => {
    ...
    Object.defineProperty(i, "name", Xt);
...
var getSignals = () => {
  ...
  return [ ...tr, ...e ].map(normalizeSignal);
};
...
var makeError = ({stdout: e, stderr: t, all: r, error: n, signal: i, exitCode: a, command: D, escapedCommand: c, timedOut: l, isCanceled: d, killed: f, parsed: {options: {timeout: h, cwd: p = s.cwd()}}}) => {
  var m = void 0 === (i = null === i ? void 0 : i) ? void 0 : rr[i].description;
...
var normalizeStdio = e => {
  ...
  if ((e => nr.some((t => void 0 !== e[t])))(e)) {
    throw new Error(`It's not possible to provide \`stdio\` in combination with one of ${nr.map((e => `\`${e}\``)).join(", ")}`);
  }
  if ("string" == typeof t) {
    return t;
  }
  ...
  var r = Math.max(t.length, nr.length);
  return Array.from({
    length: r
  }, ((e, r) => t[r]));
};
...
ir.push("SIGHUP", "SIGINT", "SIGTERM");

if ("win32" !== process.platform) {
  ir.push("SIGALRM", "SIGABRT", "SIGVTALRM", "SIGXCPU", "SIGXFSZ", "SIGUSR2", "SIGTRAP", "SIGSYS", "SIGQUIT", "SIGIOT");
}

if ("linux" === process.platform) {
  ir.push("SIGIO", "SIGPOLL", "SIGPWR", "SIGSTKFLT");
}
...
var ar = Symbol.for("signal-exit emitter");

var ur = globalThis;

var or = Object.defineProperty.bind(Object);
...
var sr = globalThis.process;

var {onExit: Dr} = (cr = processOk(sr) ? new class SignalExit extends SignalExitBase {
  #e="win32" === sr.platform ? "SIGINT" : "SIGHUP";
  #t=new Emitter;
  #r;
  #n;
  #i;
...
}(sr) : new class SignalExitFallback extends SignalExitBase {
  onExit() {
    return () => {};
  }
  load() {}
...
var getChunkType = e => {
  ...
  if (Number.isInteger(e.byteLength) && Number.isInteger(e.byteOffset) && "[object ArrayBuffer]" === lr.call(e.buffer)) {
    return "typedArray";
  }
  return "others";
};
...
var getNewContentsLength = e => fr ** Math.ceil(Math.log(e) / Math.log(fr));
...
async function getStreamAsBuffer(e, t) {
  ...
  try {
    return arrayBufferToNodeBuffer(await async function getStreamAsArrayBuffer(e, t) {
      return getStreamContents(e, hr, t);
    }(e, t));
  } catch (e) {
...
var {PassThrough: vr} = w;

var mr = getDefaultExportFromCjs((function() {
  var e = [];
  var t = new vr({
    objectMode: !0
  });
...
var gr = (async () => {})().constructor.prototype;

var Cr = [ "then", "catch", "finally" ].map((e => [ e, Reflect.getOwnPropertyDescriptor(gr, e) ]));
...
var getEscapedCommand = (e, t) => normalizeArgs(e, t).map((e => (e => {
  if ("string" != typeof e || Er.test(e)) {
    return e;
  }
  return `"${e.replaceAll('"', '\\"')}"`;
})(e))).join(" ");

var Fr = b.debuglog("execa").enabled;
...
var yr = (() => {
  ...
  var u = (e = "", t = 0) => {
    ...
    var s = 0 === t ? Ve.green(ct) : 1 === t ? Ve.red(st) : Ve.red(Dt);
    process.stdout.write(Ge.cursor.move(-999, 0)), process.stdout.write(Ge.erase.down(1)), 
  ...
  process.on("SIGINT", (() => $(1))), process.on("SIGTERM", (() => $(1))), process.on("exit", $), 
  {
    start: (D = "") => {
      i = !0, r = function WD({input: e = s.stdin, output: t = s.stdout, overwrite: r = !0, hideCursor: n = !0} = {}) {
        ...
        return n && process.stdout.write(Ge.cursor.hide), e.once("keypress", C), () => {
          e.off("keypress", C), n && process.stdout.write(Ge.cursor.show), e.isTTY && !at && e.setRawMode(!1), 
          i.terminal = !1, i.close();
        };
      }(), a = D.replace(/\.+$/, ""), process.stdout.write(`${Ve.gray(dt)}\n`);
      var c = 0, l = 0;
      n = setInterval((() => {
        var t = Ve.magenta(e[c]), r = ".".repeat(Math.floor(l)).slice(0, 3);
        process.stdout.write(Ge.cursor.move(-999, 0)), process.stdout.write(Ge.erase.down(1)), 
...
async function run(n) {
  ((e = "") => {
    process.stdout.write(`${Ve.gray(lt)}  ${e}\n`);
  })("GQL.Tada");
  var i = await question("Where can we get your schema? Point us at an introspection JSON-file, a GraphQL schema file or an endpoint", (async t => {
    try {
      var i = new URL(t);
      yr.start("Validating the URL.");
      try {
        var a = await fetch(i.toString());
        if (!a.ok) {
          yr.stop("Validated the URL.");
          return !!await se({
  ...
  if (s) {
    ...
    yr.stop("Installed packages.");
  } else {
    yr.start("Writing to package.json.");
    try {
      ...
      yr.stop("Written to package.json.");
    } catch (e) {
      yr.stop('Failed to write to package.json, you can try adding "gql.tada" and "@0no-co/graphqlsp" yourself.');
    }
  }
  yr.start("Writing to tsconfig.json.");
  try {
    var d = e.resolve(n, "tsconfig.json");
    var f = await t.readTSConfigFile(d);
    var h = i.endsWith(".json") || i.endsWith(".graphql");
    f.compilerOptions = {
      ...f.compilerOptions,
      plugins: [ {
        name: "@0no-co/graphqlsp",
        schema: h ? e.relative(n, i) : i,
        tadaOutputLocation: e.relative(n, a)
      } ]
    };
    await r.writeFile(d, JSON.stringify(f, null, 2));
  } catch (e) {}
  yr.stop("Written to tsconfig.json.");
  ((e = "") => {
    process.stdout.write(`${Ve.gray(dt)}\n${Ve.gray(ft)}  ${e}\n\n`);
  })("Off to the races!");
}
...
var wr = Object.assign((async function _main() {
  var e = new Cli({
