...
import { buildClientSchema as a, buildSchema as n, executeSync as t } from "graphql";

import { Kind as i, GraphQLError as o, parse as s, print as u, OperationTypeNode as l } from "@0no-co/graphql.web";

import * as c from "node:fs/promises";
...
var rehydrateGraphQlError = e => {
  if (e && e.message && (e.extensions || "GraphQLError" === e.name)) {
    ...
    return new o(e.message, e.nodes, e.source, e.positions, e.path, e, e.extensions || {});
  } else {
    return new o(e);
  }
};
...
var stringify = e => {
  ...
  if (!n.length && e.constructor && Object.getPrototypeOf(e).constructor !== Object.prototype.constructor) {
    var t = y.get(e) || Math.random().toString(36).slice(2);
    y.set(e, t);
    return stringify({
      __key: t
    });
...
var extract = (e, r, a) => {
  if (null == a || "object" != typeof a || a.toJSON || h.has(a)) {} else if (Array.isArray(a)) {
    for (var n = 0, t = a.length; n < t; n++) {
      extract(e, `${r}.${n}`, a[n]);
    }
  } else if (a instanceof E || a instanceof k) {
    e.set(r, a);
  } else {
    h.add(a);
...
var replaceOutsideStrings = (e, r) => r % 2 == 0 ? e.replace(g, "\n") : e;

var sanitizeDocument = e => e.split(N).map(replaceOutsideStrings).join("").trim();
...
var stringifyDocument = e => {
  ...
  if ("string" == typeof e) {
    ...
    r = T.get(e) || sanitizeDocument(u(e));
    T.set(e, r);
  }
...
var A = "undefined" != typeof TextDecoder ? new TextDecoder : null;

var I = /boundary="?([^=";]+)"?/i;

var b = /data: ?([^\n]+)/;

var toString = e => "Buffer" === e.constructor.name ? e.toString() : A.decode(e);
...
var O = function Client(e) {
  ...
  var makeResultSource = e => {
    var r = takeUntil(filter((r => "teardown" === r.kind && r.key === e.key))(u.source))(filter((r => r.operation.kind === e.kind && r.operation.key === e.key && (!r.operation.context._instance || r.operation.context._instance === e.context._instance)))(h));
    if ("query" !== e.kind) {
      ...
      r = switchMap((r => {
        ...
        return r.stale || r.hasNext ? a : merge([ a, map((() => {
          r.stale = !0;
          return r;
        }))(take(1)(filter((r => r.key === e.key))(u.source))) ]);
      }))(r);
    }
    if ("mutation" !== e.kind) {
      r = onEnd((() => {
  ...
  var d = Object.assign(c, {
    ...
    createRequestOperation(e, a, n) {
      ...
      return makeOperation(e, a, {
        ...
        ...s,
        ...n,
        requestPolicy: n.requestPolicy || s.requestPolicy,
...
var makeIntrospectionQuery = e => {
  if (S && L === e) {
    ...
    return S = _makeIntrospectionQuery(L = e);
  }
};
...
function loadFromSDL(e) {
  ...
  var s = null;
  var load = async () => {
    var i = r.extname(e.file);
    var o = await d.readFile(e.file, {
      encoding: "utf8"
    });
    if (".json" === i) {
      var s = JSON.parse(o);
      ...
      var l = makeIntrospectionQuery(x);
      var c = t({
        schema: u,
        document: l
      });
    ...
    notifyOnUpdate(r) {
      if (!i.size) {
        (async () => {
          ...
          var r = d.watch(e.file, {
            signal: o.signal,
            persistent: !1
          });
          try {
            for await (var a of r) {
              if (s = await load()) {
                for (var n of i) {
                  n(s);
                }
...
var F = [ "name", "tadaOutputLocation", "tadaTurboLocation", "tadaPersistedLocation" ];
...
var parseSchemaConfig = (r, a) => {
  ...
  if (!("schema" in r) || "string" != typeof r.schema) {
    throw new TadaError("Schema is missing a `schema` property");
  } else if ("tadaOutputLocation" in r && r.tadaOutputLocation && "string" != typeof r.tadaOutputLocation) {
    throw new TadaError("Schema contains a `tadaOutputLocation` property, but it's not a file path");
  } else if ("tadaTurboLocation" in r && r.tadaTurboLocation && "string" != typeof r.tadaTurboLocation) {
    throw new TadaError("Schema contains a `tadaTurboLocation` property, but it's not a file path");
  } else if ("tadaPersistedLocation" in r && r.tadaPersistedLocation && "string" != typeof r.tadaPersistedLocation) {
    throw new TadaError("Schema contains a `tadaPersistedLocation` property, but it's not a file path");
  }
  var i = r;
  var o = i.schema;
  if ("string" == typeof o) {
...
var stat = (e, r = isFile) => c.stat(e).then(r).catch((() => !1));

var R = "undefined" != typeof require ? require.resolve.bind(require) : p(import.meta.url).resolve;

var resolveExtend = async (e, r) => {
  try {
    return toTSConfigPath(R(e, {
      paths: [ r ]
    }));
  } catch (e) {
    return null;
...
var toTSConfigPath = r => ".json" !== e.extname(r) ? e.resolve(_, r, "tsconfig.json") : e.resolve(_, r);

var readTSConfigFile = async e => {
  var r = toTSConfigPath(e);
  var a = await c.readFile(r, "utf8");
  var n = f.parseConfigFileTextToJson(r, a);
  if (n.error) {
    throw new TSError(n.error);
  }
  return n.config || {};
};

var findTSConfigFile = async r => {
  var a = toTSConfigPath(r || _);
...
var printIntrospectionType = e => {
  if ("ENUM" === e.kind) {
    ...
    return `{ kind: 'INPUT_OBJECT'; name: ${printName(e.name)}; inputFields: ${a}; }`;
...
var C = [ "/* eslint-disable */", "/* prettier-ignore */" ].join("\n") + "\n";
...
function outputIntrospectionFile(e, r) {
  if (/\.d\.ts$/.test(r.fileType)) {
    ...
    var n = [ C, P, `export type introspection = ${a};\n`, "import * as gqlTada from 'gql.tada';\n" ];
    if (!("name" in e) || !e.name) {
    ...
    return [ C, q, `const introspection = ${t} as const;\n`, "export { introspection };" ].join("\n");
  }
